...
export default function DetailsScreen() {
  ...
  const { user } = useAuth(); // 2. Pegando o usuário logado (ou null)
  
  const { id, title, price, location, description, image } = useLocalSearchParams();

  // FUNÇÃO QUE EXECUTA A RESERVA COM TRAVA DE LOGIN
  const handleReserve = () => {
    if (!id) return;

    // 3. VERIFICAÇÃO: Se não estiver logado, bloqueia a reserva
    if (!user) {
      Alert.alert(
        "Perfil necessário",
        ...
        [
          ...
          { 
            text: "Fazer Login", 
            onPress: () => router.push('../login') // Envia para a tela de login
          }
        ]
      );
    ...
    // 4. Se estiver logado, o código continua normalmente
